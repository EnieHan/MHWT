...
from .cross_attention_stability import RSTB
...
class PatchEmbed(nn.Module):
    ...
    Args:
        img_size   (int | tuple): Image size.  Default: 224.
...
class MHWT(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_fusion = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)
        self.conv_fusion_complement = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)
        ...
        # #########-------fusion_layer-------##########
        self.fusion_layer = RSTB(dim=embed_dim,
                                 ...
                                 img_size=img_size,
                                 patch_size=patch_size)
        self.fusion_norm = norm_layer(self.num_features)  # 180
        # #########-------fusion_layer-------##########
    ...
    # #########-------forward_fusion-------##########
    def forward_fusion(self, x, c):  # after first_conv trans
        size = (x.shape[2], x.shape[3])  # [B, C, H, W]
        x = self.patch_embed(x)  # [B, H*W, C]
        c = self.patch_embed(c)

        x = self.fusion_layer(x, c, size)

        x = self.fusion_norm(x)
        x = self.patch_unembed(x, size)
        return x
        # #########-------forward_fusion-------##########
    ...
    def forward(self, x, complement, mask):
        ...
        x_first = self.conv_fusion(x)
        c_first = self.conv_fusion_complement(complement)

        x_fusion = self.forward_fusion(x_first, c_first)

        x_trans = self.forward_features(x_fusion, x_copy, mask)

        x_trans = self.conv_after_body(x_trans) + x_fusion
...
if __name__ == '__main__':
    x = torch.rand(1, 1, 192, 256)
    y = torch.rand(1, 1, 192, 256)
    z = torch.rand(1, 1, 192, 256)
    model = MHWT(img_size=(192, 256),
                 in_chans=1,
                 embed_dim=120,
                 depths=(8, 8, 8, 8, 8, 8),
                 num_heads=(4, 4, 4, 4, 4, 4),
